...
and mismatches between leaf constraints and subtrees are ignored. The output
consists of the extended `constraints` and `interface` with renumbered
variables (in particular, numbering of original variables in `constraints` is
retained). Constraints in both trees retain their bounds.
...
"""
function inject_interface(constraints::C.ConstraintTree, interface::C.ConstraintTree)
    ...
    go(cs::C.Constraint, iface::C.Constraint) =
        C.Constraint(value = cs.value + iface.value, bound = cs.bound)
